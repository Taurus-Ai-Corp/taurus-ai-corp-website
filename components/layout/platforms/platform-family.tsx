...
export function PlatformFamily({ family, index }: PlatformFamilyProps) {
  ...
  // Expand production by default; if none, show beta as primary row
  const primary =
    core.length > 0 ? core : family.subProducts.filter((p) => p.stage === "beta" || p.stage === "lab").slice(0, 4);
  const rest =
    core.length > 0
      ? secondary
        ...
        <div className="flex gap-4">
          ...
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-muted-foreground mono-label text-xs">
                0{index + 1}
              </span>
